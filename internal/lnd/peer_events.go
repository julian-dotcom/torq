...
func SubscribePeerEvents(ctx context.Context,
	...
	for {
		...
		eventNodeId := cache.GetChannelPeerNodeIdByPublicKey(peerEvent.PubKey, nodeSettings.Chain, nodeSettings.Network)
		if eventNodeId != 0 {
...
func ImportPeerStatusFromLnd(ctx context.Context,
	...
	for _, peer := range resp.Peers {
		peerNodeId := cache.GetChannelPeerNodeIdByPublicKey(peer.PubKey, nodeSettings.Chain, nodeSettings.Network)
		if peerNodeId == 0 {
