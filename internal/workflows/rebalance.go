...
type Rebalancer struct {
	...
	Request         lightning_helpers.RebalanceRequest
}
...
func (runner *RebalanceRunner) addFailedHop(
	hopSourcePublicKey string,
	hopDestinationPublicKey string,
	amountMsat uint64) {

	runner.FailedHops[hopSourcePublicKey+"_"+hopDestinationPublicKey] = amountMsat
}

func (runner *RebalanceRunner) isFailedHop(
	hopSourcePublicKey string,
	hopDestinationPublicKey string,
	amountMsat uint64) bool {

	failedHopAmountMsat, exists := runner.FailedHops[hopSourcePublicKey+"_"+hopDestinationPublicKey]
	return exists &&
		core.GetDeltaPerMille(failedHopAmountMsat, amountMsat) <
			...
			if len(pendingRebalancers) > 0 {
				...
				for {
					...
					if pendingRebalancer.Request.IncomingChannelId != 0 {
						log.Debug().Msgf(
							"Rebalancer timed out after %s for Origin: %v, OriginId: %v, Incoming Channel: %v",
							runningFor, pendingRebalancer.Request.Origin, pendingRebalancer.Request.OriginId,
							pendingRebalancer.Request.IncomingChannelId)
					}
					if pendingRebalancer.Request.OutgoingChannelId != 0 {
						log.Debug().Msgf(
							"Rebalancer timed out after %s for Origin: %v, OriginId: %v, Outgoing Channel: %v",
							runningFor, pendingRebalancer.Request.Origin, pendingRebalancer.Request.OriginId,
							pendingRebalancer.Request.OutgoingChannelId)
					}
					i++
				}
...
func RebalanceRequests(ctx context.Context,
	db *sqlx.DB,
	requests lightning_helpers.RebalanceRequests,
	nodeId int) []lightning_helpers.RebalanceResponse {
	...
	if incoming && outgoing {
		...
		return []lightning_helpers.RebalanceResponse{{
			Request:               lightning_helpers.RebalanceRequest{},
			CommunicationResponse: lightning_helpers.CommunicationResponse{},
		}}
	}
	responses := make(map[int]lightning_helpers.RebalanceResponse)
	for _, request := range requests.Requests {
	...
	if incoming {
		for _, request := range requests.Requests {
			...
			if len(finalChannelIds) == 0 {
				responses[request.IncomingChannelId] = lightning_helpers.RebalanceResponse{
					Request: request,
					CommunicationResponse: lightning_helpers.CommunicationResponse{
			...
			if !addRebalancer(rebalancer) {
				rebalanceResponse := lightning_helpers.RebalanceResponse{
					Request: request,
					CommunicationResponse: lightning_helpers.CommunicationResponse{
				...
				rebalanceResponse.Message = fmt.Sprintf(
					"IncomingChannelId: %v already has a running rebalancer "+
						"for origin: %v, originId: %v with reference number: %v",
					rebalancer.Request.IncomingChannelId,
		...
		for _, request := range requests.Requests {
			...
			if len(finalChannelIds) == 0 {
				responses[request.OutgoingChannelId] = lightning_helpers.RebalanceResponse{
					Request: request,
					CommunicationResponse: lightning_helpers.CommunicationResponse{
						Status: lightning_helpers.Inactive,
						Error:  "No channelIds found after filtering based on historic records",
			...
			if !addRebalancer(rebalancer) {
				...
				rebalanceResponse.Message = fmt.Sprintf(
					"OutgoingChannelId: %v already has a running rebalancer "+
						"for origin: %v, originId: %v with reference number: %v",
					rebalancer.Request.OutgoingChannelId,
					rebalancer.Request.Origin, rebalancer.Request.OriginId, rebalancer.Request.OriginReference)
	...
	var res []lightning_helpers.RebalanceResponse
...
func (rebalancer *Rebalancer) start(
	...
	log.Debug().Msgf("Rebalance initiated for origin: %v, originReference: %v, "+
		"incomingChannelId: %v, outgoingChannelId: %v",
		rebalancer.Request.Origin, rebalancer.Request.OriginReference,
		rebalancer.Request.IncomingChannelId, rebalancer.Request.OutgoingChannelId)
	if rebalancer.Request.IncomingChannelId != 0 {
		incomingChannel := cache.GetChannelSettingByChannelId(rebalancer.Request.IncomingChannelId)
		if incomingChannel.Capacity == 0 || incomingChannel.Status != core.Open {
			log.Error().Msgf("IncomingChannelId is invalid "+
				"for origin: %v, originReference: %v and incomingChannelId: %v",
	...
	if rebalancer.Request.OutgoingChannelId != 0 {
		outgoingChannel := cache.GetChannelSettingByChannelId(rebalancer.Request.OutgoingChannelId)
		if outgoingChannel.Capacity == 0 || outgoingChannel.Status != core.Open {
			log.Error().Msgf("OutgoingChannelId is invalid "+
				"for origin: %v, originReference: %v and outgoingChannelId: %v",
	...
	rebalancer.Status = core.Active
	latestResult, err := rebalances.GetLatestResultByOrigin(db, rebalancer.Request.Origin, rebalancer.Request.OriginId,
		rebalancer.Request.IncomingChannelId, rebalancer.Request.OutgoingChannelId, core.Active,
		rebalancePreviousSuccessResultTimeoutMinutes)
	if err != nil {
		log.Error().Err(err).Msgf("Obtaining latest result "+
	...
	if previousSuccess.Hops != "" {
		log.Debug().Msgf("Previous success found "+
			"for origin: %v, originReference: %v, incomingChannelId: %v, outgoingChannelId: %v",
			rebalancer.Request.Origin, rebalancer.Request.OriginReference,
			rebalancer.Request.IncomingChannelId, rebalancer.Request.OutgoingChannelId)
		...
		if result.Status == core.Active {
			log.Debug().Msgf("Previous success successfully reused "+
				"for origin: %v, originReference: %v, incomingChannelId: %v, outgoingChannelId: %v",
				rebalancer.Request.Origin, rebalancer.Request.OriginReference,
				rebalancer.Request.IncomingChannelId, rebalancer.Request.OutgoingChannelId)
			removeRebalancer(rebalancer)
		...
		log.Debug().Msgf("Previous success reuse failed "+
			"for origin: %v, originReference: %v, incomingChannelId: %v, outgoingChannelId: %v",
			rebalancer.Request.Origin, rebalancer.Request.OriginReference,
			rebalancer.Request.IncomingChannelId, rebalancer.Request.OutgoingChannelId)
	}
...
func (rebalancer *Rebalancer) convertPreviousSuccess(previousSuccess rebalances.RebalanceResult) rebalances.RebalanceResult {
	if rebalancer.Request.OutgoingChannelId != 0 &&
		!slices.Contains(rebalancer.Request.ChannelIds, previousSuccess.IncomingChannelId) {

		log.Debug().Msgf("Previous success ignored as it's not available anymore "+
			"for origin: %v, originReference: %v, incomingChannelId: %v, outgoingChannelId: %v",
			rebalancer.Request.Origin, rebalancer.Request.OriginReference,
			rebalancer.Request.IncomingChannelId, rebalancer.Request.OutgoingChannelId)
	...
	if rebalancer.Request.IncomingChannelId != 0 &&
		!slices.Contains(rebalancer.Request.ChannelIds, previousSuccess.OutgoingChannelId) {
...
func (rebalancer *Rebalancer) createRunner(
	...
	if rebalancer.Status == core.Inactive {
		return
	}
	...
	if channelId == 0 {
		...
		if rebalancer.Request.IncomingChannelId != 0 {
			log.Debug().Msgf("Pending Outgoing ChannelIds got exhausted "+
				"for Origin: %v, OriginId: %v, IncomingChannelId: %v (%s)",
				rebalancer.Request.Origin, rebalancer.Request.OriginId, rebalancer.Request.IncomingChannelId, runningFor)
		}
		if rebalancer.Request.OutgoingChannelId != 0 {
			log.Debug().Msgf("Pending Incoming ChannelIds got exhausted "+
				"for Origin: %v, OriginId: %v, OutgoingChannelId: %v (%s)",
		...
		rebalancer.Status = core.Pending
		if !addRebalancer(rebalancer) {
			if rebalancer.Request.IncomingChannelId != 0 {
	...
	if result.Status == core.Active {
		removeRebalancer(rebalancer)
		runningFor := time.Since(rebalancer.ScheduleTarget).Round(1 * time.Second)
		msg := fmt.Sprintf("Successfully rebalanced after %s %vmsats @ %vmsats (%v ppm) "+
			"using incomingChannelId: %v, outgoingChannelId: %v",
		...
		log.Debug().Msgf("%v for Origin: %v, OriginId: %v (Hops: %v)",
			msg, rebalancer.Request.Origin, rebalancer.Request.OriginId, result.Hops)
		rebalancer.Status = core.Inactive
		return
	}
	runner.Cancel()
	runner.Status = core.Inactive
	...
	for _, workflowNode := range unfocusedPath {
		...
		case workflow_helpers.WorkflowNodeDataSourceTorqChannels:
			...
			case "all", "eventXorAll":
				...
				if rebalancer.Request.IncomingChannelId != 0 {
					incomingChannelSettings := cache.GetChannelSettingByChannelId(rebalancer.Request.IncomingChannelId)
				...
				if rebalancer.Request.OutgoingChannelId != 0 {
					outgoingChannelSettings := cache.GetChannelSettingByChannelId(rebalancer.Request.OutgoingChannelId)
				...
				if torqNodeId != 0 {
					channelIds = cache.GetChannelStateNotSharedChannelIds(torqNodeId, true)
				}
			case "event":
				msg := fmt.Sprintf(
...
outer:
	for _, channelId := range channelIds {
		...
		if channelSettings.Capacity == 0 || channelSettings.Status != core.Open {
			continue outer
		}
		channelState := cache.GetChannelState(torqNodeId, channelId, true)
...
func (rebalancer *Rebalancer) addRunner(
	channelId int,
	runnerCtx context.Context,
	runnerCancel context.CancelFunc) *RebalanceRunner {
...
func (runner *RebalanceRunner) getRoutes(
	...
	outgoingChannel := cache.GetChannelSettingByChannelId(runner.OutgoingChannelId)
	incomingChannel := cache.GetChannelSettingByChannelId(runner.IncomingChannelId)
	var remoteNode cache.NodeSettingsCache
	if outgoingChannel.FirstNodeId == nodeId {
		remoteNode = cache.GetNodeSettingsByNodeId(incomingChannel.SecondNodeId)
	} else {
		remoteNode = cache.GetNodeSettingsByNodeId(incomingChannel.FirstNodeId)
	}
	...
	if err != nil {
		return nil, errors.Wrapf(err,
			"QueryRoutes for outgoing nodeId: %v, publicKey: %v", outgoingChannel.FirstNodeId, remoteNode.PublicKey)
	}
...
func (runner *RebalanceRunner) pay(
	...
	if result.Status == lnrpc.HTLCAttempt_FAILED {
		rebalanceResult.Status = core.Inactive
		...
		if result.Failure.Code == lnrpc.Failure_TEMPORARY_CHANNEL_FAILURE {
			rebalanceResult.Status = core.Pending
	...
	rebalanceResult.Status = core.Active
	if result != nil && result.Route != nil {
...
func (runner *RebalanceRunner) validateRoute(nodeId int, route *lnrpc.Route) bool {
	previousHopPublicKey := cache.GetNodeSettingsByNodeId(nodeId).PublicKey
...
func validateRebalanceRequest(request lightning_helpers.RebalanceRequest) *lightning_helpers.RebalanceResponse {
...
func verifyNotZeroUint(request lightning_helpers.RebalanceRequest, value uint64, label string) *lightning_helpers.RebalanceResponse {
	if value == 0 {
		return &lightning_helpers.RebalanceResponse{
			Request: request,
			CommunicationResponse: lightning_helpers.CommunicationResponse{
...
func verifyNotZeroInt(request lightning_helpers.RebalanceRequest, value int64, label string) *lightning_helpers.RebalanceResponse {
	if value == 0 {
		return &lightning_helpers.RebalanceResponse{
			Request: request,
			CommunicationResponse: lightning_helpers.CommunicationResponse{
...
func updateExistingRebalanceRequest(db *sqlx.DB, request lightning_helpers.RebalanceRequest) *lightning_helpers.RebalanceResponse {
	rebalancer := getRebalancer(request.Origin, request.OriginId, request.IncomingChannelId, request.OutgoingChannelId)
	if rebalancer == nil {
		return nil
	}
	if rebalancer.Request.IncomingChannelId != request.IncomingChannelId ||
		rebalancer.Request.OutgoingChannelId != request.OutgoingChannelId {
	...
	if rebalancer.Request.IncomingChannelId != 0 {
		rebalanceResponse.Message = fmt.Sprintf(
			...
			rebalancer.Request.IncomingChannelId, rebalancer.Request.OriginReference)
	}
	if rebalancer.Request.OutgoingChannelId != 0 {
		rebalanceResponse.Message = fmt.Sprintf(
			"Updated existing rebalancer for origin: %v with originId: %v and OutgoingChannelId: %v (ref: %v)",
			rebalancer.Request.Origin, rebalancer.Request.OriginId,
			rebalancer.Request.OutgoingChannelId, rebalancer.Request.OriginReference)
	}
	return rebalanceResponse
}

func setRebalancer(db *sqlx.DB, request lightning_helpers.RebalanceRequest, rebalancer *Rebalancer) error {
