...
type PeerNode struct {
	...
	DaysSinceLastConnected int                         `json:"daysSinceLastConnected" db:"days_since_last_connected"`
...
func GetPeerNodes(db *sqlx.DB, network core.Network) ([]PeerNode, error) {
	...
	SELECT
		...
		nch.setting,
		last_disconnection.disconnected_since AS date_last_connected,
		CASE
		    WHEN last_disconnection.disconnected_since IS NULL THEN 0
		    ELSE CURRENT_DATE::DATE - last_disconnection.disconnected_since::DATE
		END days_since_last_connected
	FROM Node n
	LEFT JOIN (
