...
func getOnChainTxs(db *sqlx.DB,
	nodeIds []int,
	filter sq.Sqlizer,
	order []string,
	limit uint64,
	offset uint64) (r []*Transaction, total uint64, err error) {

	r, err = runQuery(db, nodeIds, filter, order, limit, offset, r, true)
	if err != nil {
		return r, 0, err
	}

	r, err = runQuery(db, nodeIds, filter, order, limit, offset, r, false)
	if err != nil {
		return r, 0, err
	}
	...
	totalQb := sq.Select("count(*) as total").
		...
		FromSelect(
			...
			`).
				PlaceholderFormat(sq.Dollar).
				From("tx").
				Where(sq.Eq{"tx.node_id": nodeIds}, sq.NotEq{"tx.label": nil}),
			"subquery").
		Where(filter)
...
func runQuery(db *sqlx.DB, nodeIds []int, filter sq.Sqlizer, order []string, limit uint64, offset uint64, r []*Transaction, withLabel bool) ([]*Transaction, error) {
	qb := getQuery(nodeIds, filter, order, limit, offset, withLabel)
	// Compile the query
	qs, args, err := qb.ToSql()
	if err != nil {
		return nil, errors.Wrap(err, "SQL compile statement")
	}

	rows, err := db.Queryx(qs, args...)
	if err != nil {
		return nil, errors.Wrap(err, "Run SQL Query")
	}
	defer rows.Close()
	for rows.Next() {
		var tx Transaction
...
func getQuery(nodeIds []int, filter sq.Sqlizer, order []string, limit uint64, offset uint64, withLabel bool) sq.SelectBuilder {
	//language=PostgreSQL
		...
		qb = sq.Select("*").
			...
			FromSelect(
				sq.Select(`
				   ...
				   total_fees,
				   label,
				   '' as lnd_tx_type_label,
				   '' as lnd_short_chan_id
				`).
					PlaceholderFormat(sq.Dollar).
					From("tx").
					Where(sq.Eq{"tx.node_id": nodeIds}, sq.Eq{"tx.label": nil}),
				"subquery").
			Where(filter).
			OrderBy(order...)
	}
