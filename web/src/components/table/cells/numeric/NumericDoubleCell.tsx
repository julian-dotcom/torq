...
import {
  ArrowStepIn16Regular as ArrowIn,
  ArrowStepOut16Regular as ArrowOut,
...
function NumericDoubleCell({ local, remote, suffix, className }: numericDoubleCell) {
  ...
  return (
    <div className={classNames(styles.cell, styles.numericCell, className)}>
      <div className={styles.local}>{localValue} {suffix} <ArrowOut className={styles.inboundIcon} /></div>
      <div className={classNames(styles.remote, styles.outbound)}>{remoteValue} {suffix} <ArrowIn  className={styles.outboundIcon} /></div>
    </div>
  );
}
